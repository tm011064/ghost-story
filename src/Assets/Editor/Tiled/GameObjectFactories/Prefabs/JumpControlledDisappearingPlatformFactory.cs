...
public class JumpControlledDisappearingPlatformFactory : AbstractGameObjectFactory
{
  private const string PrefabName = "Jump Controlled Disappearing Platform Controller";

  public JumpControlledDisappearingPlatformFactory(
    GameObject root,
    Map map,
  ...
  protected virtual GameObject CreatePrefabFromGameObject(TiledGroupConfig config)
  {
    ...
    return CreateInstantiableObject(
       ...
       PrefabName,
       config,
       new JumpControlledDisappearingPlatformInstantiationArguments
       {
         MaxVisiblePlatforms = config.Group.GetPropertyValueAsInt32("MaxVisiblePlatforms"),
         Platforms = platforms
       });
  }
}
