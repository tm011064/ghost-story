...
namespace Tiled2Unity
{
  partial class ImportTiled2Unity
  {
    ...
    private void AssignLayerTo(GameObject gameObject, XElement xml, ImportBehaviour importComponent)
    {
      ...
      if (layerId == -1)
      {
        importComponent.RecordWarning("Layer '{0}' is not defined for '{1}'. Check project settings in Edit->Project Settings->Tags & Layers", layerName, GetFullGameObjectName(gameObject.transform));
        return;
      }
    ...
    private void AssignTagTo(GameObject gameObject, XElement xml, ImportBehaviour importComponent)
    {
      ...
      {
        importComponent.RecordWarning("Tag '{0}' is not defined for '{1}'. Check project settings in Edit->Project Settings->Tags & Layers", tag, GetFullGameObjectName(gameObject.transform));
      }
    }
